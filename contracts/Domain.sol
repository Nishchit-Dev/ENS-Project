...
contract Domains is ERC721URIStorage {
    ...
    string public tld; // this is  last part of domain ex 0xGoku.eth
    string svgPartOne =
    ...
    // ex: 0xGoku => 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266
    // name  =>    address owns or zero Address

    mapping(string => string) public records;

    // ex: 0xGoku => www.thepixelverse.xyz
    // name =>  any type of data or string can be stored

    constructor(string memory _tld) payable ERC721("eth name service", "ENS") {
        tld = _tld;
        console.log("contract has been Deployed", _tld);
    }
    ...
    function registerDomain(string memory name) public payable {
        // checking if the name is registered or not
        require(domains[name] == address(0));

        uint256 _price = price(name);

        require(msg.value >= _price, "You Dont have enough Matic");

        // creating final ENS name
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name":"',
                _name,
                '","description":"A domain on the eth name service","image":"data:image/svg+xml;base64,',
    ...
    function setRecord(string memory name, string memory record) public {
        require(
            domains[name] == msg.sender,
            "you are not the owner of this domains"
        );
        records[name] = record;
    }
